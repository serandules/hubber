...
Drone.prototype.start = function (done) {
    ...
    var close = function (code, signal) {
        log.error('error starting drone %s code: %s, signal: %s', that.id, code, signal);
        if (log.debug) {
            summary(that);
        }
        done(true);
    ...
    var error = function () {
        log.error('error starting drone %s code: %s, signal: %s', that.id);
        if (log.debug) {
            summary(that);
        }
        done(true);
    ...
    this.app.once('started', function (port, pid) {
        that.status = 'started';
